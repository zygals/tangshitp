...
//        dump($request);exit;
        ...
        if ($admin) {
            if ($admin->type == '店铺管理员') {
                if ($admin->status != '正常') {
                    $this->error('帐号禁用或删除,请联系平台！');
                }
            }
            $admin->setInc('times');
            session('admin_tangs', (object)array('name' => $admin->name, 'id' => $admin->id, 'type' => $admin->type/*,'shop_id' => $admin->shop_id,'privilege'=>$admin->privilege*/));
//            dump(session('admin_tangs'));exit;
